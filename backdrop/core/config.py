...
def load(app, name):
    ...
    load_config_file(app, '{}.py'.format(GOVUK_ENV))
    load_config_file(app, '{}/{}.py'.format(name, GOVUK_ENV))
...
def load_legacy_config_file(app, name, env):
    ...
    path = os.path.join(ROOT_PATH, 'backdrop', name, '{}.py'.format(env))
    if os.path.exists(path):
        app.config.from_object('backdrop.{}.{}'.format(name, env))
